...
public class MemoryPanel extends JLabel {    //状态条的颜色
    ...
    public void paint(Graphics g) {
        ...
        int top = insets.top;          //得到内存使用状态信息
        Runtime runtime = Runtime.getRuntime();
        long freeMemory = runtime.freeMemory();
        long totalMemory = runtime.totalMemory();          //组件（JPanel）除去左、右边框后的宽度
        int insideWidth = dimension.width - (insets.left + insets.right) - 16;     //内存使用量的显示宽度
        ...
        int stringHeight = fontmetrics.getHeight() - fontmetrics.getDescent();     //显示文字
